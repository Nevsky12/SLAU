...
TEST(TestJacobi, Test) {
    ...
    double Tolerance = 0.71;

    auto res = Slae::Solvers::Jacobi(TestMatrix, b, InitialState, Tolerance);
    ASSERT_EQ(AnaliticSolve.size(), res.size()) << "Vectors solve and res are of unequal length";
    for (int i = 0; i < res.size(); ++i) {
        EXPECT_EQ(AnaliticSolve[i], res[i])
                            << "Jacobi gives another result than correct analitics resullt with this tolerance: "
                            << Tolerance << ".  Jacobi method result: "
